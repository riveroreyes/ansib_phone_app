...
Exercise 1: Create an endpoint to retrieve the phone catalog, and pricing. (Para pruebas, las rutas no estan autentificadas)
	...
	Test response Api con curl:
		curl -i -H "Accept:application/json" -H "Content-Type:application/json" "http://0.0.0.0:8000/api/product"
...
Exercise 2: Create endpoints to check and create an order.	(Para pruebas, las rutas no estan autentificadas)	
	...
	Test response Api con curl:
		curl -i -H "Accept:application/json" -H "Content-Type:application/json" -X POST http://0.0.0.0:8000/api/order -d '{"name":"Cheila","surname":"Bautista","email":"cheilabautista@hotmail.com","phones":[{"id":2,"quantity":2},{"id":3,"quantity":2},{"id":5,"quantity":1},{"id":6,"quantity":1},{"id":8,"quantity":1},{"id":9,"quantity":1},{"id":10,"quantity":1}]}'	
